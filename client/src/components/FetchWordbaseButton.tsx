...
const FetchWordbaseButton: FC = () => {
    ...
    const fetchWordbase = async (): Promise<void> => {
        ...
        try {
            ...
            setError(error instanceof Error ? error.message : 'Failed to fetch the application wordbase.');
        } finally {
            setLoading(false);
        }
    }
    ...
    return (
        <>
            ...
            { message && <Typography variant="body1" color="success" sx={ { alignSelf: 'center' } }>{ message }</Typography> }
            { error && <Typography variant="body1" color="error" sx={ { alignSelf: 'center' } }>{ error }</Typography> }
        </>
    )
}
